...
import { TPMRecord } from "@/lib/types";
import { parseISO, format, isValid } from "date-fns";

interface ChartTPMProps {
  data: TPMRecord[];
}
...
export function ChartTPM({ data }: ChartTPMProps) {
  ...
  sortedData.forEach((record) => {
    ...
    existing[record.request_model] = parseInt(record.TPM); // Ensure TPM is a number
  });
        ...
        >
          ...
          <Tooltip
            formatter={(value, name) => [
              value,
              name === "TPM" ? "Tokens per Minute" : name,
            ]}
